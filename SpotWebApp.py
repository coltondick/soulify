...
# Adding additional imports to support the use of environment variables
import configparser
...
def load_spotify_auth() -> tuple[str, str, str]:
    """
    Resolve CLIENT_ID/SECRET/REDIRECT_URI with priority:
      1) Docker env vars (SPOTIFY_* first, then plain)
      2) /config/spotifyauth.conf (supports dashed & underscored keys)
      3) Default redirect from BASE_URL or localhost
    """
    conf_path = os.path.join(CONFIG_DIR, "spotifyauth.conf")
    conf = _read_kv_file(conf_path)

    # Accept both legacy (spotify-id/redirect-uri) and new (spotify_client_id/redirect_uri)
    cid_file   = conf.get("spotify_client_id")   or conf.get("spotify-id")     or ""
    sec_file   = conf.get("spotify_client_secret") or conf.get("spotify-secret") or ""
    redir_file = conf.get("redirect_uri")        or conf.get("redirect-uri")    or ""

    # Env priority: SPOTIFY_* first, then plain
    client_id    = (os.getenv("SPOTIFY_CLIENT_ID")    or os.getenv("CLIENT_ID")    or cid_file)
    client_secret= (os.getenv("SPOTIFY_CLIENT_SECRET")or os.getenv("CLIENT_SECRET")or sec_file)
    redirect_uri = (os.getenv("SPOTIFY_REDIRECT_URI") or os.getenv("REDIRECT_URI") or redir_file)

    if not redirect_uri:
        redirect_uri = (BASE_URL.rstrip("/") + "/callback") if BASE_URL else f"http://localhost:{FLASK_PORT}/callback"
...
def ensure_valid_token():
    """Return a valid access token if present/refreshable; otherwise None."""
...
def login():
    """Spotify login route."""
    CLIENT_ID_EFF, CLIENT_SECRET_EFF, REDIRECT_URI_EFF = load_spotify_auth()
    app.logger.info(f"[auth] Using CLIENT_ID={CLIENT_ID_EFF[:6]}… REDIRECT_URI={REDIRECT_URI_EFF}")
    params = {
        "response_type": "code",
        "client_id": CLIENT_ID_EFF,
        "scope": (SCOPE or ""),
        "redirect_uri": REDIRECT_URI_EFF,
    }
...
def callback():
    ...
    token_response = requests.post(
        ...
        headers={
            'Authorization': 'Basic ' + base64.b64encode(f"{CLIENT_ID_EFF}:{CLIENT_SECRET_EFF}".encode()).decode(),
            'Content-Type': 'application/x-www-form-urlencoded'
        }
    )
...
def refresh_spotify_token():
    """Utility to refresh the Spotify access token."""
    CLIENT_ID_EFF, CLIENT_SECRET_EFF, _ = load_spotify_auth()
    refresh_token = session.get('refresh_token')
    headers = {
        'Authorization': 'Basic ' + base64.b64encode(f"{CLIENT_ID_EFF}:{CLIENT_SECRET_EFF}".encode()).decode(),
        'Content-Type': 'application/x-www-form-urlencoded'
...
def read_soulify_conf():
    ...
    if os.path.exists(soulify_conf_path):
        ...
        try:
            ...
            update_library = cfg.getboolean(
                'PostDownloadProcessing', 'UpdateLibraryMetadataAndRefreshJellyfin', fallback=False
            )
        except Exception as e:
            logging.error(f"Error reading soulify.conf: {e}")
